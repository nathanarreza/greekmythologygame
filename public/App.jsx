...
const STARTER_CHARACTERS = [
    {
        ...
        name: "Zeus",
        canFly: true,
        stats: { HP: 140, STR: 14, MAG: 20, WIS: 17 },
        passive: {
            name: "Panhellenios’ Rule",
            text:
                "Can set Thunderstorm hazard (end of round: flying ≠ Zeus take 8, grounded take 4). Once per challenge, First Judgement: Zeus goes first. Olympian Authority: MAG check vs <19 to forfeit action.",
        },
        abilities: [
            {
                id: "ZEUS_ARCING",
                ...
                power: 14,
                type: MAG,
                text: "Stun target for their next turn. Pierces shields/temp HP.",
                effects: [makeEffect({ key: "stun", duration: 1, note: "Arcing Bolt" })],
            },
            {
                id: "ZEUS_FURY",
                ...
                power: 15,
                type: MAG,
                text: "4 bolts. If first hits, rest cannot be dodged (abstracted as +5 true)",
                bonus: () => 5,
            },
            {
                id: "ZEUS_TITANSTORM",
                ...
                power: 19,
                type: MAG,
                text: "Stun 1 turn + lingering shock: -3 HP at start of next 2 turns. Accuracy -3; if hit, Zeus goes first next round.",
                effects: [makeEffect({ key: "stun", duration: 1, note: "Titanstorm" })],
            },
        ],
    },
    {
        id: "HERC",
        name: "Hercules",
        stats: { HP: 150, STR: 20, MAG: 4, WIS: 10 },
        passive: { name: "Twelve-Labors Resilience", text: "Immune to stun/fear/etc. Below 50 HP next attack +5 dmg." },
        abilities: [
            {
                id: "HERC_SLAM",
                name: "Lion’s Slam",
            ...
            },
            {
                ...
                power: 15,
                type: PHY,
                text: "All enemies fall prone (next physical vs them +5).",
                effects: [makeEffect({ key: "weaken", power: 0, duration: 1, note: "Prone: next physical +5" })],
            },
            {
                id: "HERC_LABORS",
                name: "The 12 Labors",
                power: 20,
                type: PHY,
                text: "Halve ATK/HP for 2 turns; then reborn 1.75x stats (except poison weakness). (Scripted lightly)",
                effects: [makeEffect({ key: "special_labors", duration: 2 })],
            },
        ],
    },
    {
        id: "ARES",
        name: "Ares",
        stats: { HP: 150, STR: 20, MAG: 4, WIS: 7 },
        passive: { name: "Battle Frenzy", text: "+2/+5/+10 base dmg thresholds; counter on melee; Warcry taunt synergy." },
        abilities: [
            { id: "ARES_WARCRY", name: "Warcry", power: 0, type: PHY, text: "Taunts all; -25% dmg taken by Ares (vs taunted).", effects: [makeEffect({ key: "taunt", duration: 1, note: "All -> Ares" })] },
            { id: "ARES_SPEAR", name: "Spear of Blood", power: 15, type: PHY, text: "+5 power if under Warcry.", bonus: (ctx) => (ctx.self.lastCast === "ARES_WARCRY" ? 5 : 0) },
            { id: "ARES_WRATH", name: "God of War’s Wrath", power: 25, type: PHY, text: "Set HP to 1; huge AoE scaling by HP lost (capped). (Abstracted)" },
        ],
    },
    {
        id: "APOLLO",
        name: "Apollo",
        canFly: true,
        stats: { HP: 125, STR: 9, MAG: 20, WIS: 18 },
        passive: { name: "Solar Presence", text: "+7 MAG aura; buffs via music; fast movement; many immunities." },
        abilities: [
            { id: "APO_ARROW", name: "Blazing Arrow", power: 10, type: MAG, text: "Pierces magical shields; +10 if target used magic last turn." },
            { id: "APO_LYRIC", name: "Lyric of Healing / Plague", power: 12, type: MAG, text: "Heal/Plague toggle (pick in UI)." },
            { id: "APO_SUNBURST", name: "Sunburst Prophecy", power: 20, type: MAG, text: "Blind all 1 turn + burn 10 for 2 turns (20 if target under debuff)." },
        ],
    },
    {
        id: "ARTEMIS",
        name: "Artemis",
        canFly: true,
        stats: { HP: 130, STR: 13, MAG: 15, WIS: 17 },
        passive: { name: "Moonlit Precision", text: "Never misses vs conditioned targets; stealthy; detects traps." },
        abilities: [
            { id: "ART_LUNAR", name: "Lunar Arrow", power: 12, type: PHY, text: "Pierces DR; anti-heal 1 round.", effects: [makeEffect({ key: "antiHeal", duration: 1 })] },
            { id: "ART_SNARE", name: "Snare Shot", power: 12, type: PHY, text: "Lose movement/flying; -2 next physical.", effects: [makeEffect({ key: "bind", duration: 1 })] },
            { id: "ART_HUNT", name: "Wild Hunt", power: 20, type: PHY, text: "2 spectral strikes: split 10/10 or 20 single." },
        ],
    },
    {
        id: "HADES",
        name: "Hades",
        stats: { HP: 95, STR: 10, MAG: 20, WIS: 18 },
        passive: { name: "Lord of the Dead", text: "Once per game stay at 1 HP for a round; many immunities." },
        abilities: [
            { id: "HAD_SOUL", name: "Soul Bind", power: 12, type: MAG, text: "Target cannot heal next round.", effects: [makeEffect({ key: "antiHeal", duration: 1 })] },
            { id: "HAD_GRASP", name: "Underworld Grasp", power: 16, type: MAG, text: "Up to 2 enemies lose movement; -4 STR for 2 turns.", effects: [makeEffect({ key: "bind", duration: 2 })] },
            { id: "HAD_BLACK", name: "Black Riches", power: 20, type: MAG, text: "AoE: 5 now + 2 HP lost next turn.", effects: [makeEffect({ key: "burn", power: 2, duration: 1, note: "Black Riches" })] },
        ],
    },
    {
        id: "HYPNOS",
        name: "Hypnos",
        stats: { HP: 120, STR: 4, MAG: 20, WIS: 19 },
        passive: { name: "Dreamweaver’s Aura", text: "Targets gain Drowsy (-2) after his turn; sleep synergies." },
        abilities: [
            { id: "HYP_DROWSY", name: "Drowsy Mist", power: 8, type: MAG, text: "Apply Drowsy (-2 rolls)", effects: [makeEffect({ key: "weaken", power: 2, duration: 1 })] },
            { id: "HYP_LULLABY", name: "Lullaby of Oblivion", power: 12, type: MAG, text: "Sleep 1 turn; breaks on damage.", effects: [makeEffect({ key: "sleep", duration: 1 })] },
            { id: "HYP_EMBRACE", name: "Sleep’s Embrace", power: 20, type: MAG, text: "AoE sleep 1 turn; break on dmg; retaliation behavior vs Hypnos.", effects: [makeEffect({ key: "sleep", duration: 1 })] },
        ],
    },
    {
        id: "THANATOS",
        name: "Thanatos",
        canFly: true,
        stats: { HP: 125, STR: 12, MAG: 20, WIS: 19 },
        passive: { name: "Embrace of Death", text: "Enemies < 40 HP vs Thanatos are instantly defeated; self-heal +25 on defeat." },
        abilities: [
            { id: "THA_BIND", name: "Soul Bind", power: 10, type: MAG, text: "Stun 1 turn.", effects: [makeEffect({ key: "stun", duration: 1 })] },
            { id: "THA_CORRUPT", name: "Corrupt Demise", power: 12, type: MAG, text: "On kill: summon ally soul (abstracted)." },
            { id: "THA_FINAL", name: "Silent Finality", power: 20, type: PHY, text: "On kill: target can’t be revived this session.", effects: [makeEffect({ key: "reviveLock", duration: 99 })] },
        ],
    },
];
            ...
            <div className="max-w-7xl mx-auto grid gap-4">
                ...
                {state.phase === "draft" && (
                    <div className="flex items-center gap-3">
                        <button disabled={state.teams[0].length !== 4 || state.teams[1].length !== 4} onClick={startBattle} className="px-4 py-2 rounded-2xl bg-indigo-600 disabled:opacity-30">Start Battle</button>
